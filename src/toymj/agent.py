...
class MultiClassSGDRegressor:
    ...
    def __init__(self, n_classes, rng):
        self._models = [
    ...
    def partial_fit(self, X, Y):
        ...
        logger.info(f'partial_fit({X.shape}, {Y.shape})')
...
class Agent:
    ...
    def __init__(self, rule, gamma, eps, rng):
        ...
        self.model = MultiClassSGDRegressor(self.n_actions, self.rng)
        n_random_states = self.n_actions * 2
        self.model.fit(
                numpy.asarray(rule.random_states(n_random_states, self.rng)),
